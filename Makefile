...
#---------------------------------------------------------------------------------
TARGET		:=	Lynchy_Adventure
BUILD		:=	build
SOURCES		:=	src src/entities src/systems src/components
DATA		:=	assets/data
INCLUDES	:=	inc
GRAPHICS	:=	assets/gfx
#GFXBUILD	:=	$(BUILD)
...
APP_TITLE       :=  Lynchy Adventure
APP_DESCRIPTION :=  A simple 2d retro zelda-like game.
APP_AUTHOR      :=  SylStarz
...
#---------------------------------------------------------------------------------
debug:
	@rm -f $(TARGET).3dsx $(OUTPUT).smdh $(TARGET).elf
	@rm -f $(BUILD)/*.o
	$(MAKE) DEBUG=1
#---------------------------------------------------------------------------------
undebug:
